...
def send_visits():
    ...
    # Query parameters থেকে uid এবং region নেওয়া
    uid = request.args.get("uid")
    server = request.args.get("region", "IND").upper()  # default IND
    ...
    async def main():
        nonlocal first_response, player_info
        connector = aiohttp.TCPConnector(limit=0)
        async with aiohttp.ClientSession(connector=connector) as session:
            tasks = [visit(session, token) for token in tokens]
            results = await asyncio.gather(*tasks)
            ...
            for success, resp in results:
                if success:
                    total_success += 1
                    if first_response is None and resp:
                        first_response = resp
                        player_info = parse_protobuf_response(resp)

            return total_success, len(tokens), player_info

    total_success, total_sent, player_info = asyncio.run(main())
